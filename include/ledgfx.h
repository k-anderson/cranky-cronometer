...
void DrawPixels(float fPos, float count, CRGB color)
{
  ...
  float remaining = min(count, 44 - fPos);
  int iPos = fPos;
  ...
  if (remaining > 0.0f)
  {
    effect_leds[translate[iPos++]] += ColorFraction(color, amtFirstPixel);
  ...
  {
    effect_leds[translate[iPos++]] += color;
    remaining--;
  }
  ...
  if (remaining > 0.0f)
  {
    effect_leds[translate[iPos]] += ColorFraction(color, remaining);
  }
}
