    ...
    virtual void Draw()
    {
        if (_fadeRate != 0)
        {
            for (size_t i = 0; i < _cLength; i++)
                effect_leds[translate[i]].fadeToBlackBy(_fadeRate);
        }
        ...
        for (size_t i = 0; i < _cBalls; i++)
        {
            ...
            CRGB color = ColorFromPalette(gCurrentPalette, i * (255 / _cBalls), 255);
            ...
            effect_leds[translate[position]] += color;
            effect_leds[translate[position + 1]] += color;

            if (_bMirrored)
            {
                effect_leds[translate[_cLength - 1 - position]] += color;
                effect_leds[translate[_cLength - position]] += color;
            }
        }
    }
};